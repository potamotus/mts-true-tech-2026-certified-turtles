...
// Create an MWS GPT API client (OpenAI-compatible)
const config = new Configuration({
  apiKey: process.env.MWS_API_KEY,
  basePath: process.env.MWS_API_BASE || "https://api.gpt.mws.ru/v1",
});
...
export async function POST(req: Request) {
  // Ask MWS GPT for a streaming completion given the prompt
  const response = await openai.createChatCompletion({
    model: process.env.MWS_MODEL || "gpt-4o-mini",
    stream: true,
    temperature: 0,
